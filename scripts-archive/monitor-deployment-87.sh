#!/bin/bash
# Monitor deployment #87 actively
# Run ID: 21057742193
...
REGION="us-east-1"
LOG_FILE="deployment-87-monitor.log"

echo "=== Deployment #87 Monitoring Started ===" | tee -a "$LOG_FILE"
echo "Run ID: $RUN_ID" | tee -a "$LOG_FILE"
echo "Fix: Docker Hub credentials populated in AWS Secrets Manager" | tee -a "$LOG_FILE"
    ...
    if [ -n "$cluster" ]; then
        ...
        if [ -n "$services" ]; then
            ...
            if [ "$stopped_tasks" -gt 0 ]; then
                ...
                stopped_task_arn=$(AWS_PAGER="" aws ecs list-tasks \
                    --cluster "$cluster_name" \
                    --region "$REGION" \
                    --desired-status STOPPED \
                    --query 'taskArns[0]' \
                ...
                if [ -n "$stopped_task_arn" ] && [ "$stopped_task_arn" != "None" ]; then
                    stopped_reason=$(AWS_PAGER="" aws ecs describe-tasks \
                        --cluster "$cluster_name" \
                        --region "$REGION" \
                        --tasks "$stopped_task_arn" \
                        --query 'tasks[0].stoppedReason' \
                        --output text 2>/dev/null || echo "Unknown")
                    echo "    Stopped reason: $stopped_reason" | tee -a "$LOG_FILE"
                fi
            fi
        fi
    else
