...
import { signIn, setUpTOTP, verifyTOTPSetup, confirmSignIn } from 'aws-amplify/auth';
...
const Login: React.FC = () => {
    ...
    const initiateTOTPSetup = async () => {
        ...
        try {
            ...
            console.log('TOTP Details received');
            setTotpSecretCode(totpSetupDetails.sharedSecret);
            const appName = "Antigravity Case System";
            const setupUrl = `otpauth://totp/${encodeURIComponent(appName)}:${encodeURIComponent(email)}?secret=${totpSetupDetails.sharedSecret}&issuer=${encodeURIComponent(appName)}`;
            ...
            >
                ...
                <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                    ...
                    <h2 className="text-xl font-semibold text-white mb-6">
                        {mfaSetup ? 'Setup 2-Factor Auth' : 'Sign In'}
                    </h2>
                        ...
                        {mfaSetup ? (
                            <motion.form key="mfa-form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} onSubmit={handleVerifyTOTP} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-slate-400 ml-1">Verification Code</label>
